...
- **매출/정산(`SalesRepository`)**: 도메인 `SalesSummary`(합계·요일별·인기메뉴·정산)는 집계가 필요하나 서버는
  ...
  ⚠ **2026-06-23 라이브 스펙(`/v3/api-docs`) 확인**: `StorePaymentsRes` 는 `{ total, payments:[Item] }` 이고
  `Item` 은 `{ menuId, quantity, optionIds }` 뿐 — **금액·일시(amount/paidAt) 필드가 전혀 없다**. 따라서
  `totalSales`(결제금액 합)·`dailySales`(일자 버킷)·`topMenus`(메뉴별 매출) 등 `SalesSummary` 핵심값을 클라에서도
  **산출 불가**. 서버가 payments 에 금액·일시를 추가하거나 매출 집계 엔드포인트를 제공하기 전까지 **Mock 유지 확정**
  (phase 22 step 3 deferred).
