...
- Figma Variable Collection: **Colors** (등록 토큰 9개: `canvas`, `ink`, `body`, `muted`, `muted-soft`, `primary`, `surface-card`, `on-primary`, `error`)
- 아직 미등록(모바일 적용 시 필요하면 동일 명/값으로 **Figma Variable에 추가 등록** 후 사용): `surface-dark`, `hairline`, `primary-hover`, `accent-soft`, `on-dark`, `success`, `warning`
